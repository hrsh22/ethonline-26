...
describe("collector shell navigation", () => {
  ...
  it("keeps utilities in the footer and out of permanent primary chrome", async () => {
    ...
    ).toEqual(["/learn#help", "/auction", "/learn", "/status", "/faucet"]);
  });
  ...
  it("keeps funding in the footer without adding a second header row", async () => {
    await renderShell();
    expect(
      container.querySelector("header [data-collector-funding-row]"),
    ).toBeNull();
    expect(container.querySelector('header a[href="/faucet"]')).toBeNull();
    expect(container.querySelector('footer a[href="/faucet"]')).not.toBeNull();
  });
