...
export interface SpacecraftArtProps {
...
/** One drawing for the collection, Fleet, detail and social images. */
export function SpacecraftArt(props: SpacecraftArtProps) {
  const instanceId = useId().replaceAll(":", "");
  return <SpacecraftDrawing {...props} instanceId={instanceId} />;
}

/** Pure drawing also usable by the social-image renderer. */
const serialGlyphs: Record<string, string[]> = {
...
/** Path lettering works in the browser and the social-image SVG renderer. */
const serialPath = (identityId: number) =>
  [...`O/${String(identityId).padStart(4, "0")}`]
    .flatMap((character, index) =>
      (serialGlyphs[character] ?? []).flatMap((row, y) =>
        [...row].flatMap((pixel, x) =>
          pixel === "1"
            ? [`M${176 + index * 8 + x * 2} ${194 + y * 2}h1.5v1.5h-1.5z`]
            : [],
        ),
      ),
    )
...
export function SpacecraftDrawing({
  className,
  decorative = false,
  identityId,
  label,
  ...
  rewardTrack,
  instanceId,
}: SpacecraftArtProps & { readonly instanceId: string }) {
  ...
  return (
    ...
    >
      ...
      <g className="craft-core">
        ...
        <path fill="#d8d8d3" d={serialPath(identityId)} />
      </g>
    </svg>
  );
}
