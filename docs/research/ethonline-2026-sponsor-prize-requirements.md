...
**Eligibility is unresolved:** the user selected Start Fresh and has now confirmed that some project work preceded kickoff. The deployment document also describes a **September 1** redeployment, and the indexer ADR records **August 29** acceptance. Document the exact prior-work baseline and obtain the organizers' classification before claiming eligibility. Redeploying contracts after kickoff does not change when the code, design, or assets were created; neither does a later initial repository commit. Preserve truthful provenance and dates. No registration change, organizer contact, or redeployment was performed. [Deployment record](../deployments/base-sepolia-84532.md), [ADR 0009](../adr/0009-own-a-typescript-sqlite-historical-read-model.md), [ETHGlobal prior-work rules](https://ethglobal.com/rules).

Classic/Start Fresh excludes pre-existing project-specific code, designs, and assets; public libraries and starter kits are allowed. Approved Continuity tracks permit prior codebases but require substantive event-period work and disclosure of the baseline. New extensions must remain open source. ETHGlobal requires written disclosure of prior work plus repository/video/description evidence; undisclosed prior work can invalidate awards. Preserve meaningful commit history. Continuity eligibility varies by sponsor. An unmarked bounty should not be assumed accessible to Continuity entrants merely because it lacks a restriction badge. [ETHGlobal rules](https://ethglobal.com/rules), [event getting-started guide](https://ethglobal.com/events/ethonline2026/info/start).

**If organizers classify the project as Continuity:** Uniswap explicitly offers two $1,000 awards. The Graph explicitly offers its AI category at $2,500/$1,500/$1,000, but an indexing-only project does not thereby qualify for AI; any added reasoning workflow must improve the product. Ask specifically whether the composition category and Privy's unmarked categories accept Continuity submissions. Chainlink's $500 upgrade could be relevant only for a meaningful new event-period upgrade, not unchanged VRF. Do not add an artificial AI feature to recover a prize category. The technical shortlist remains conditional on both useful implementation and confirmed pool access. [Uniswap requirements](https://ethglobal.com/events/ethonline2026/prizes/uniswap-foundation), [Graph pool definitions](https://thegraph.com/blog/hackathon-resources/), [Privy requirements](https://ethglobal.com/events/ethonline2026/prizes/privy), [Chainlink upgrade](https://ethglobal.com/events/ethonline2026/prizes/chainlink).
