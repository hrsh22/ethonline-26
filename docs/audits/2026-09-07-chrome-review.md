...
- The development badge intercepted taps on mobile More. Disabling the normal floating badge restores the app control, but a later live rejection exposed a separate development error indicator that can still intercept taps; see the [live verification follow-up](2026-09-07-live-reown-verification.md#validation-and-limits).
...
- Wallet connection and Trade actions could drop keyboard focus when a pending update made their focused button natively disabled. Those actions now remain focusable while disabled, with activation still blocked and disabled styling preserved. The browser detector distinguishes a real loss of focus inside the page from native browser-chrome traversal.
...
At phone width, verified More navigation, Relic inspection, browser Back to Relics, invalid identity lookup feedback, successful lookup of #1639, copy-link feedback, and wallet-display help. The wallet chooser fitted the viewport. Closing it retained the entered Trade amount. No wallet signature or transaction was submitted. The app tab recorded no console errors during this walkthrough. A final phone-home check after the font correction confirmed a 56 px header, no horizontal overflow at 375 px, and no console errors. After history recovery, the advanced market chart opened with interval, indicator and drawing controls.
...
The final web suite passed 884 tests; configuration passed 137 tests and `pnpm check` passed. The final production build passed the 110-case browser matrix and all 17 harness self-tests after the font and focus corrections. The four formerly failing CI focus cases also passed with 4× CPU slowdown. Independent Standards and Spec reviews found no remaining actionable findings. CI also exposed a faucet-test scheduling race; it now waits for the actual recovery state and preserves its one-proof/one-request assertions.
